...
const {
  ...
  web: {
    recaptcha_v2: { site_key, secret_key },
  },
...
module.exports = async (req, b = null) => {
  ...
  if (data.description.length > max_summary_length)
    return { success: false, message: "Your summary is too long." };
  if (String(data.note).length > max_summary_length)
  ...
  if (data.tags) {
    ...
    if (data.tags.length > max_bot_tags)
      return {
        success: false,
        message: `Select up to ${max_bot_tags} tags max`,
      };
  ...
  try {
    ...
    else return { success: false, message: "Could not find user" };
  }
  ...
  try {
    ...
    if (users.length > max_owners_count)
      return {
        success: false,
        message: `You can only add up to ${max_owners_count} additional owners`,
      };
